...
document.addEventListener("click", (event) => {
  if (!event.target.closest(".select") && isOpen.value) {
    isOpen.value = false;
  }
});
</script>
...
<template>
  <div class="select">
    <label for="select" class="select__label" v-if="label">
      {{ label }}
    </label>
    <button
      type="button"
    ...
    <div class="select__trigger" @click="isOpen = false" v-show="isOpen"></div>
  </div>
</template>
